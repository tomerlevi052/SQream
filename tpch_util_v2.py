...
def create_new_schema():
    ...
    try:
        ...
        file_scheme_handle = open(SCHEMA_FILE_PATH, "r")
        ...
        for curr_line in file_scheme_handle:
            ...
            if "CREATE TABLE" in curr_line:
                ...
                table_token = curr_line.split()[2]
                print(f"{table_token}") # just for stdout debugging
        ...
        file_scheme_handle.seek(0)

        # Create the tables from the schema file
        cursor.execute(file_scheme_handle.read())
...
def fetch_results():
    try:
        ...
        # Check if results table even exist # TODO: You need a side function for this!!
        check_if_table_has_no_rows_query = f"SELECT COUNT(*) FROM {RESULTS_TABLE_NAME};"
        cursor.execute(check_if_table_has_no_rows_query)

        has_rows = cursor.fetchone()[0] #  [0] == 0  (false)?
        ...
        if has_rows:
            ...
            print(f"{RESULTS_TABLE_NAME} exist, but has no rows in it.. nothing to fetch for")

            # print the empty columns names
            cursor.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = '{RESULTS_TABLE_NAME}'")

            # Get the result from the ? returned
...
def export_results_to_csv():
    ...
    print(f"Exporting results from {RESULTS_TABLE_NAME}...")
    try:
        # String to hold the CSV file name
        csv_file_name = "export_result_csv.csv"

        # create a new csv file to export to, and open it to get a handle to the file object
        with open(csv_file_name, "w", newline = "") as csv_file_handle:

            # Create a csv writer object to write to the file
            writer_header = csv.writer(csv_file_handle, delimiter = ",")

            # Write the header row with all needed header columns
            writer_header.writerow(["run_datetime", "tcph_query_name", "benchmark_results"])

            # Copy from the table name into this csv file
            cursor.copy_expert(f"COPY {RESULTS_TABLE_NAME} TO STDOUT WITH DELIMITER ',' CSV", csv_file_handle)

            print(f"Files exported to {csv_file_name} successfully!")

        # commit changes ()
        # TODO: There were no changes made directly in database only copying (read only) from results table
...
def clean_results_table():
    ...
    try:
        ...
        check_if_table_exists_query = f"SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = "f"'{RESULTS_TABLE_NAME}');"
        cursor.execute(check_if_table_exists_query)

        # the results returned as a tuple of booleans in size of 1 element.. ->  [0] will get us this element which is the boolean value of the query
        table_exists = cursor.fetchone()[0]

        # case met? -> just return without creating it
        if table_exists:
            # If it exists DELETE/UPDATE it in order to clean it from all values (column remains)
            cursor.execute(f"DELETE FROM {RESULTS_TABLE_NAME}")

            print('Results table is cleaned and deleted')
            # Flush buffers Run all transactions and save the changes to the database
            db_conn.commit()

        # else -> results table doesn't exist, need to create one before cleaning it
        else:
            print(f"{RESULTS_TABLE_NAME} doesn't exist, need to create one before cleaning it")

    # end of the exception block
    except psycopg2.OperationalError as exception_object:
        sys.exit(f"{Fore.RED}Error in cleaning the results table operation results to file: {exception_object}{Style.RESET_ALL}")
...
if __name__ == "__main__":
    ...
    print it""")
    parser.add_argument('--export-results', action='store_true', help="""Fetch the results and export them into a CSV file""")
    parser.add_argument('--clean-results', action='store_true', help="""Fetch the results and export them into a CSV file""")

    # Parse the command-line arguments passed to the script and return their stored values (parse_args returns a namespace object)
    ...
    # other options
        ...
        print("No argument provided. Use --create-schema, --load-data, --run-benchmark, --save-results, "
              "--fetch-results, --export-results")
